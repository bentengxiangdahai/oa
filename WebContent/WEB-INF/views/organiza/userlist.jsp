...
<div id="scrollContent">
	<table class="tableStyle" >
		...
		<%
			Page<User> p=(Page<User>)request.getAttribute("page");
...
<div style="height:35px;">
	<div class="float_right padding5 paging">
		<div class="float_left padding_top4">
			...
			%>
				<span>			
				<a href="queryuser?departmentId=${departmentId }&nowpage=<%=currentPage-1%>">上一页</a>
				</span>
			<%
				}else{
			%>
		...
		%>
		<span><a href="queryuser?departmentId=${departmentId }&nowpage=0">1</a></span>
		<span><a href="queryuser?departmentId=${departmentId }&nowpage=1">2</a></span>
		<span><a href="queryuser?departmentId=${departmentId }&nowpage=2">3</a></span>
		<span><a href="queryuser?departmentId=${departmentId }&nowpage=3">4</a></span>
		<span><a href="queryuser?departmentId=${departmentId }&nowpage=4">5</a></span>
		<span>...</span>
		<span><a href="queryuser?departmentId=${departmentId }&nowpage=<%=totalPage-1 %>"><%=totalPage %></a></span>
		<%
			}else{
				for(int i=0;i<totalPage;i++){
		%>
			<span><a href="queryuser?departmentId=${departmentId }&nowpage=<%=i%>"><%=(i+1) %></a></span>
		<%
				}
			}
		%>
			...
			%>
				<span>
				<a href="queryuser?departmentId=${departmentId }&nowpage=<%=currentPage+1%>">下一页</a>
				</span>
			<%
				}else{
			%>
